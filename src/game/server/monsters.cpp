...
#if 1
edict_t* CBaseMonster::GetEnemyPtr(void)
{
	return m_hEnemy.Get();
}
void CBaseMonster::SetEnemyPtr(edict_t* pEnemy)
{
	m_hEnemy.Set(pEnemy);
}

void CBaseMonster::SetBloodColor(int bloodColor)
{
	m_bloodColor = bloodColor;
}

float CBaseMonster::GetNextAttack(void) const
{ 
	return m_flNextAttack; 
}

void CBaseMonster::SetNextAttack(float flNextAttack)
{
	m_flNextAttack = flNextAttack; 
}

void CBaseMonster::SetCapabilities(int afCapabilities)
{
	if (afCapabilities > (1 << bits_CAP_LAST))
	{
		ALERT(at_error, "No support for capability/capabilities with value(s): %d.\n", afCapabilities);
		return;
	}

	m_afCapability = afCapabilities;
}

void CBaseMonster::AddCapability(int afCapability)
{
	if (afCapability > (1 << bits_CAP_LAST))
	{
		ALERT(at_error, "No support for capability/capabilities with value(s): %d.\n", afCapability);
		return;
	}

	m_afCapability |= afCapability;
}

void CBaseMonster::RemoveCapability(int afCapability)
{
	if (afCapability > (1 << bits_CAP_LAST))
	{
		ALERT(at_error, "No support for capability/capabilities with value(s): %d.\n", afCapability);
...
int CBaseMonster::GetCapabilities(void) const
{
	return m_afCapability;
}

BOOL CBaseMonster::HasCapabilities(int afCapabilities) const
{
	if (afCapabilities > (1 << bits_CAP_LAST))
	{
		ALERT(at_error, "No support for capability/capabilities with value(s): %d.\n", afCapabilities);
	...
	return (m_afCapability & afCapabilities) ? TRUE : FALSE;
}

void CBaseMonster::ClearCapabilities(void)
{
	m_afCapability &= ~m_afCapability;
}

void CBaseMonster::SetFieldOfView(float fov)
{
	m_flFieldOfView = fov;
}

float CBaseMonster::GetFieldOfView(void) const
{
