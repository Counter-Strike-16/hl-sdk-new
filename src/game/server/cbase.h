...
//	virtual void	SetActivator( CBaseEntity *pActivator ) {}
...
#if 1
	string_t	GetClassname(void) const;
	string_t	GetEntityName(void) const;
	...
	void		SetSize(const Vector& vecMins, const Vector& vecMaxs);

	void		SetViewOffset(const Vector& viewOffset);
	Vector		GetViewOffset(void) const;
	...
	void		SetAngularVelocity(const Vector& avelocity);
	Vector		GetAngularVelocity(void) const;

	void		SetRenderMode(int rendermode);
	int			GetRenderMode(void) const;

	void		SetAlpha(float renderamt);
	float		GetAlpha(void) const;

	void		SetRenderColor(const Vector& rendercolor);
	Vector		GetRenderColor(void) const;

	void		SetRenderFx(int renderfx);
	int			GetRenderFx(void)const;
...
#if 1
#define bits_CAP_LAST			bits_CAP_FLY // Change this to your last custom defined capability.
#endif
