...
#if 1
	edict_t*	GetEnemyPtr(void);
	void		SetEnemyPtr(edict_t* pEnemy);

	void		SetBloodColor(int bloodColor);

	float		GetNextAttack(void) const;
	void		SetNextAttack(float flNextAttack);

	void		SetCapabilities( int afCapabilities );
	void		AddCapability( int afCapability );
	void		RemoveCapability( int afCapability );
	int			GetCapabilities( void ) const;
	BOOL		HasCapabilities( int afCapabilities ) const;
